"""Single entry point: grower_id → full campaign brief.

Track 1 (Marketing) scope only — profile, weather, product, channel,
send date, content. No retailer/rep signals.
"""
...
from .content import ContentBrief, ContentResult, generate
...
def build_context(data: HarvestData, grower_row: pd.Series, *,
    ...
    if generate_content:
        ctx.content = generate(ctx.to_brief())
    return ctx
